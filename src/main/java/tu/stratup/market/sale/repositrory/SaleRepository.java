		...
		try {
			...
			return this.enhancedAsyncClient
					.transactWriteItems(b -> b
							.addUpdateItem(metaDataDynamoDbAsyncTable, TransactUpdateItemEnhancedRequest.builder(MetaDataItem.class)
									.item(saleMetadata)
									.conditionExpression(Expression.builder().expression("increment_id = :current_id")
											.expressionValues(Map.of(":current_id", numberValue(saleMetadata.getIncrementId())))
											.build()).build())
